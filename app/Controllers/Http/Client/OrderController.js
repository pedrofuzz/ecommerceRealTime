  ...
  async store({ request, response, auth, transform }) {
    ...
    try {
      ...
      return response.status(400).send({
        message: 'Não foi possível fazer o pedido, tente novamente'
      })
    }
  }
  ...
  async applyDiscount({ params: { id }, request, response, transform, auth }) {
    ...
    try {
      ...
      if (canAddDiscount && canApplyToOrder) {
        ...
        info.message = 'Não foi possível aplicar o cupom, tente novamente'
        info.success = false
      }
